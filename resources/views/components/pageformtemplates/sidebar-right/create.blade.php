...
<div>
    ...
    <input @titlechange.window="if ($event.detail == 'create-web-page') {$el.value = $event.detail.value.toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, '');}" name="slug" type="text" class="input input-bordered w-full" required />
</div>
